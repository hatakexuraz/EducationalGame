 ...
 * @author hatake_47
 */
public class MathsAssessment extends QuestionList {
    ...
    private void btn_nxtActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_nxtActionPerformed
        try{
            ...
            if(source.equals(btn_nxt)){                      //checks if the button is clicked
                ...
                que.addAll(maths.getAssessmentContent(l));      //Add all the values received from the function 'getContent' of MathsChapterController
                //lbl_que.setText(que.get(0));                     //set the text on 'lbl_que' which was stored in ArrayList 'que'
                answer = que.get(1);

                opt = new ArrayList<>();
                opt.addAll(maths.getAssessmentOptions(l));
    ...
    private void listQuestion() {
        try{
            ...
            que = new ArrayList<>();                             //initialize the 'que' variable of ArrayList 
            que.addAll(maths.getAssessmentContent(j));                        //Add all the values received from the function 'getContent' of GKChapterController
            lbl_que.setText(que.get(0));                         //set the text on 'lbl_que' which was stored in ArrayList 'que'
            l=2;
            ...
            opt.addAll(maths.getAssessmentOptions(j));
    ...
    public int getScore (ArrayList<String> copt, ArrayList<String> answer){ 
        int che=0;
    ...
    private void btn_finishActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_finishActionPerformed
        ...
        Score scr = new Score(point, user, unit);  //initilize Score 'scr' variable
        scr.setVisible(true);             //make the frame 'scr' visibkle
        scr.setLocation(pnl_gkmod.getWidth()-200, pnl_gkmod.getHeight()-200); //set the location of 'scr'
